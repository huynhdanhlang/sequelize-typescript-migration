...
async function getLastMigrationState(sequelize) {
    ...
    const [lastExecutedMigration] = await sequelize.query(lastExMigrationQuery, { type: sequelize_1.QueryTypes.SELECT });
    // eslint-disable-next-line @typescript-eslint/no-unused-vars
    const lastRevision = lastExecutedMigration !== undefined
        ? parseInt(lastExecutedMigration.name.split("-")[0])
        : -1;
    const [lastMigration] = await sequelize.query(lastMigrationQuery(lastRevision), { type: sequelize_1.QueryTypes.SELECT });
    if (lastMigration)
